...
describe('AvatarsController', () => {
  ...
  const mockMessage = { message: 'Avatar for userId deleted successfully' };
  const mockAvatarsService = {
    getAvatar: jest.fn().mockResolvedValue(mockAvatarBase64),
    deleteAvatar: jest.fn().mockResolvedValue(mockMessage),
  };
  ...
  describe('getAvatar', () => {
    it('should return an avatar by id', async () => {
      const result = await controller.getAvatar(2);
  ...
  describe('deleteAvatar', () => {
    it('should delete an avatar and return true', async () => {
      const result = await controller.deleteAvatar(2);
